    ...
    for (i=0; i < bucketed_data.length-3; ++i) {
        ...
        if(bgTime > mealTime) {
            ...
            if (profile.use_Static_COB_decay == true) {
                ...
                console.error("Enable Static COB decay = ", profile.use_Static_COB_decay, " COB decay adjustment factor: ", profile.cobDecayAdjust ," CR: ", profile.carb_ratio, " ISF: ", profile.sens, " CSF: ", csf );
                } else {
                    var absorbed = ci * profile.carb_ratio / sens ;
                    console.error("Enable Static COB decay = ", profile.use_Static_COB_decay);
            }
