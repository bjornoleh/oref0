...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) { 
    ...
    if (profile.tddAdjBasal == true) {
        ...
        logOutPut += startLog + bgLog + afLog + formula + "Dynamic ISF is off." + logCRratio + logCRratioLimited + weightLog;
    ...
    if (chrisFormula == false && useDynamicCR == false) {
        tddReason += "";
    } else { tddReason += basal_ratio_log + log_basalAdjust + log_isfCR + formula + afLog + limitLog; }
        ...
        iobArray.forEach(function(iobTick) {
            ...
            switch(true) {
                case chrisFormula == true && preferences.useNewFormula == true:
                    ...
                    console.log("Dynamic ISF (Log formula) )adjusted predictions for IOB and ZT: IOBpredBG: " + round(IOBpredBG,2) + " , ZTpredBG: " + round(ZTpredBG,2));
                    break;
                default:
