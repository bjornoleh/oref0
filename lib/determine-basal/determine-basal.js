...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) {
    ...
    if (weightPercentage < 1 && weightedAverage > 0) {
        tdd = weightedAverage;
        console.log("Using weighted TDD average: " + tdd.toPrecision(4) + " U, instead of past 24 h (" + tdd_before.toPrecision(2) + " U), weight: " + weightPercentage);
        weightLog = ", Weighted TDD: " + tdd.toPrecision(4) + " U";
    }
    ...
    if (chrisFormula == true && tdd > 0) {
        ...
        if (newRatio > maxLimitChris) {
            log = ", Dynamic ISF hit limitLog by autosens_max setting: " + maxLimitChris + " (" +  newRatio.toPrecision(3) + "), ";
            limitLog = ", Autosens/Dynamic Limit: " + maxLimitChris + " (" +  newRatio.toPrecision(3) + ")";
            newRatio = maxLimitChris;
        } else if (newRatio < minLimitChris) {
            log = ", Dynamic ISF hit limitLog by autosens_min setting: " + minLimitChris + " (" +  newRatio.toPrecision(3) + "). ";
            limitLog = ", Autosens/Dynamic Limit: " + minLimitChris + " (" +  newRatio.toPrecision(3) + ")";
        ...
        log += "Dynamic autosens.ratio set to " + newRatio.toPrecision(3) + " with ISF: " + isf.toPrecision(3) + " mg/dl/U (" + (isf * 0.0555).toPrecision(3) + " mmol/l/U). " + log_isfCR;
