...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime) {
    ...
    // TDD output for boxes in enacted pop up in FAX
    var tdd = 0;
    var tddReason = "";
    var tddFormula = "";
    var tddLog = "";
    ...
    if (typeof autosens_data !== 'undefined' && autosens_data) {
        ...
        const min = profile.autoseens_min;
        const max = profile.autosens_max;
        const adjustmentFactor = profile.adjustmentFactor;
        const useDynISF = profile.enableChris;
        const useDynCR = profile.enableDynamicCR;
        ...
        if (useDynISF == true || useDynCR == true) {

            if (profile.useNewFormula == true) {
                tddformula = ", Formula: Logarithmic, AF: " + adjustmentFactor;
                tdd = (sensitivityRatio * 1800) / (profile.sens * adjustmentFactor * Math.log(BG/insulinFactor+1));
                tddLog = ", TDD: " + round(tdd, 2) + " U"
            } 
            else { tdd = (sensitivityRatio * 277700) / (profile.sens * adjustmentFactor * BG);
                tddFormula = ", Formula: Original, AF: " + adjustmentFactor;
                tddLog = ", TDD: " + round(tdd, 2) + " U"
            }
            
            if (sensitivityRatio == min || sensitivityRatio == max) {
                console.error("tdd: N/A");
                tdd = 0;
                tddLog = ", TDD: N/A";
            } 
            ...
            const dynLog = ", Dynamic ISF/CR: " + dynOnOff;

            tddReason = tddLog + tddFormula + dynLog;
        }
    }
...
//    var sensitivityRatio;
    ...
    if (sensitivityRatio && profile.enableChris !== true) {
