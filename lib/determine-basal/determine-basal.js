...
var smbreason = "";  // initialize additional SMB Deleviry Ration infos for rT.reason
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        if  ( !profile.enable_BG_acceleration){
            ...
            var minmax_delta = - parabola_fit_a1/2/parabola_fit_a2 * 5;       // back frm 5min block  1 min
        ...
        console.error("1. bg_ISF adaptation is " + round(bg_ISF,2));
        ...
        if (bg_off > 0) {
            ...
            if (pp_ISF != 1) {
                ...
                if (maxISFReduction < pp_ISF) {
                    console.error("pp_ISF adaptation" + convert_bg(pp_ISF,profile) + "limited by autoisf_max", maxISFReduction);
                } else {
                    console.error("pp_ISF adaptation is" + convert_bg(pp_ISF, profile));
                }
            }
            ...
            if (delta_ISF != 1) {
                ...
                if (maxISFReduction < delta_ISF) {
                    console.error("delta_ISF adaptation " + convert_bg(delta_ISF,profile) + " limited by autoisf_max ", maxISFReduction);
                } else {
                    console.error("delta_ISF adaptation is " + convert_bg(delta_ISF, profile));
                }
            }
        }
    }
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        console.error("autoISF by-passed; avg. glucose " + avg05 + " below target " + convert_bg(target_bg,profile));
    } else {
        ...
        sens_modified = true;
        console.error("autoISF reports ISF " + convert_bg(sens,profile) + " did not do it for " + dura05 + "m; go more aggressive by " + round(levelISF,2));
    ...
    if ( sens_modified ) {
        ...
        if ( acce_ISF<1 && liftISF>1 ) {                                                                // mod V14j: brakes on for otherwise stronger ISF
            ...
            console.error("strongest ISF weakened by factor " + round(acce_ISF,2) + " as bg decelerates already");   // mod V14j: brakes on for otherwise stronger ISF
        }                                                                                               // mod V14j: brakes on for otherwise stronger ISF
        sens = round(profile.sens / liftISF, 1);
        isfreason += ", autoISF, Duration: " + dura05 + ", Avg: " + convert_bg(avg05,profile) + ", BG-ISF Ratio: " + round(bg_ISF,2) + ", Ratio: " + round(liftISF,2) + ", ISF: " + convert_bg(sens,profile) +  calcreason;
    }
...
// Adjust ISF based on sensitivityRatio
    ...
    if (typeof autosens_data !== 'undefined' && autosens_data) {
        ...
        //process.stderr.write(" (autosens ratio "+sensitivityRatio+")");
        isfreason += "Autosens, Ratio: " + sensitivityRatio + ", ISF: " + convert_bg(profile_sens,profile) + "\u2192" + convert_bg(sens,profile) + isfadaptionreason;
