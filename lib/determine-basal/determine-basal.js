...
var calcreason = "ParabolicFit:,"; // parabolic fit debugging
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    calcreason += "BG-accel: " + convert_bg(bg_acceleration,profile) + ", PF-minutes: " + parabola_fit_minutes + ", PF-corr: " + convert_bg(parabola_fit_correlation,profile) + ", PF-nextDelta: " + convert_bg(parabola_fit_next_delta,profile) + ", PF-lastDelta: " + convert_bg(parabola_fit_last_delta,profile) +  ", regular Delta: " + convert_bg(glucose_status.delta,profile);
    console.error(ppdebug + calcreason)
    calcreason = ", Parabolic Fit lastDev: " +convert_bg(parabola_fit_last_delta,profile) + ", nextDev: " + convert_bg(parabola_fit_next_delta,profile) + ", regDev: " + convert_bg(glucose_status.delta,profile);
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        console.error("bg_ISF adaptation is " + convert_bg(bg_ISF,profile));
        if (maxISFReduction < bg_ISF) {
            console.error("bg_ISF adaptation " + convert_bg(bg_ISF,profile) + " limited by autoisf_max " + maxISFReduction);
        }
        if (bg_ISF<1) {
    ...
    if ( sens_modified ) {
        ...
        isfreason += "autoISF: " + convert_bg(sens,profile) + ", Ratio: " +round(liftISF,2)  + calcreason;
    }
    return sens;
}
...
// Adjust ISF based on sensitivityRatio
    ...
    if (typeof autosens_data !== 'undefined' && autosens_data) {
        ...
        //process.stderr.write(" (autosens ratio "+sensitivityRatio+")");
        isfreason += "Autosens ISF: "+ convert_bg(profile_sens,profile) +"->" + convert_bg(sens,profile)  + ", Ratio: " + sensitivityRatio + ", ";
...
// Print summary variables based on predBGs etc.
    ...
    rT.reason = isfreason + ", FloatingCarbs: " + round(meal_data.carbs,1) + ", COB: " + rT.COB + ", Dev: " + rT.deviation + ", BGI: " + rT.BGI + ", CR: " + rT.CR + ", Target: " + rT.target_bg + ", minPredBG " + convert_bg(minPredBG, profile) + ", minGuardBG " + convert_bg(minGuardBG, profile) + ", IOBpredBG " + convert_bg(lastIOBpredBG, profile);
    if (lastCOBpredBG > 0) {
