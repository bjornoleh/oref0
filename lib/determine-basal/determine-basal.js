...
var smbreason = ""; // initialize SMB delevery calc information for rT.reason
...
{   // #### mod 7e: added switch for autoISF ON/OFF
    ...
    transreason += "BG-accel: " + round(bg_acceleration,3) + ", PF-minutes: " + parabola_fit_minutes + ", PF-corr: " + round(parabola_fit_correlation,3) + ", PF-nextDelta: " + convert_bg(parabola_fit_next_delta,profile) + ", PF-lastDelta: " + convert_bg(parabola_fit_last_delta,profile) +  ", regular Delta: " + convert_bg(glucose_status.delta,profile);
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        // start of mod V14j: calculate accel_ISF from bg acceleration and adapt ISF accordingly
        if  ( !profile.enable_BG_acceleration){
            ...
            var minmax_delta = - parabola_fit_a1/2/parabola_fit_a2 * 5;       // back from 5min block  1 min
            ...
            if ( fit_corr < 0.9 ) {
                fitreason = "by-passed accel_ISF, as correlation " + round(fit_corr,3) + " is too low";
                console.error("Parabolic fit " + fitreason);
                calcreason += ", Parabolic Fit, " + fitreason;
            } else {
                calcreason += ", Parabolic Fit, " + fitreason + ", reg\u0394: " + convert_bg(glucose_status.delta,profile) + ", last\u0394: " +convert_bg(parabola_fit_last_delta,profile) + ", next\u0394: " + convert_bg(parabola_fit_next_delta,profile) + ", Corr " + round(parabola_fit_correlation,3) + ", BG-Accel: " + round(bg_acceleration,2);
                var fit_share = 10*(fit_corr-0.9);                              // 0 at correlation 0.9, 1 at 1.00
                var cap_weight = 1;                                             // full contribution above target
                if ( glucose_status.glucose < profile.target_bg && bg_acce > 1 ) {  // corrected 13.JAN.2022 to reduce effect if acce>1
                ...
                calcreason += ", accel. Ratio: " + round(acce_ISF,2);
            }
        }
        ...
        isfreason += ", SMB Delivery Ratio:, " + round(smbRatio,2)


        var bg_ISF = 1 + interpolate(100-bg_off, profile);
        if (bg_ISF != 1) {bgisfreason = ", bg-ISF Ratio: " + round(bg_ISF,2)};
        if (maxISFReduction < bg_ISF) {
            isfadaptionreason = "bg_ISF adaptation " + round(bg_ISF,2) + ", limited by autoisf_max " + maxISFReduction;
            console.error(isfadaptionreason);
            bgisfreason = ", ltd. bg-ISF Ratio: " + maxISFReduction;
        }
        if (bg_ISF < 1) {
            if ( acce_ISF>1 ) {                                                                             // mod V14j
                bg_ISF = bg_ISF * acce_ISF;                                                                 // mod V14j
                isfadaptionreason = "bg_ISF adaptation lifted to " + round(bg_ISF,2) + ", as bg accelerates already"; // mod V14j
                console.error(isfadaptionreason);
                // bgisfreason = ", acc. bg-ISF Ratio: " + round(bg_ISF,2);
            }                                                                                               // mod V14j
            if ( bg_ISF < profile.autoisf_min ) {                                                           // mod V14j
                isfadaptionreason = "bg_ISF adaptation " + round(bg_ISF,2) + " limited by autoisf_min " + profile.autoisf_min;  // mod V14j
                console.error(isfadaptionreason);
                bgisfreason = ", ltd. bg-ISF Ratio: " + profile.autoisf_min;
            }                                                                                             // mod V14j
            sens = Math.min(720, round(profile.sens / Math.max(profile.autoisf_min, bg_ISF), 1));  // observe ISF maximum of 720?; mod V14j
            console.error("early Return autoISF:  " + convert_bg(sens,profile));
            return sens;
        } else if ( bg_ISF > 1 ) {
            sens_modified = true;
        }
        ...
        // var bg_delta = parabola_fit_next_delta;  //mod 14f
        if (bg_off > 0) {
            //var delta_ISF = 1;
            ...
            if (pp_ISF != 1) {
                ...
                if (maxISFReduction < pp_ISF) {
                    console.error("pp_ISF adaptation" + pp_ISF + "limited by autoisf_max" + maxISFReduction);
                    ppisfreason = ", pp-ISF Ratio: " + maxISFReduction;
                } else {
            ...
            if (delta_ISF != 1) {
                ...
                if (maxISFReduction < delta_ISF) {
                    console.error("delta_ISF adaptation " + delta_ISF + " limited by autoisf_max " + maxISFReduction);
                    deltaisfreason = "reg\u0394-ISF Ratio: " + maxISFReduction;
                } else {
    ...
    if (meal_data.mealCOB>0 && !profile.enableautoisf_with_COB) {
        ...
        sens_modified = true;
        console.error("dura_ISF reports ISF " + convert_bg(sens,profile) + " did not do it for " + dura05 + "m; go more aggressive by " + round(levelISF,2));
        duraisfreason = ", dura-ISF Ratio: " + round(levelISF,2);
        if (maxISFReduction < levelISF) {
            console.error("dura_ISF adaptation " + round(levelISF,2) + " limited by autoisf_max " + maxISFReduction);
            duraisfreason = ", dura-ISF Ratio: " + maxISFReduction;
        }
    }
    if ( sens_modified ) {
        ...
        isfreason += ", autoISF, Duration: " + dura05 + ", Avg: " + convert_bg(avg05,profile) + bgisfreason +  ppisfreason + deltaisfreason + duraisfreason + ", Ratio: " + round(liftISF,2) + ", ISF: " + convert_bg(sens,profile) +  calcreason;
    }
    console.error("Inside autoISF result " + convert_bg(sens,profile));
    return sens;
}
...
//    var sensitivityRatio;
    ...
    var temptarget_ratio_max = profile.temptarget_ratio_max; // limit sensitivity_ratio to temptarget_ratio_max when using low_temptarget_lowers_sensitivity 
    if (typeof profile.temptarget_ratio_max === 'undefined') {
        temptarget_ratio_max = 1.2; // default setting
    }
    if (typeof profile.temptarget_ratio_max !== 'undefined') {
        temptarget_ratio_max = Math.min(profile.temptarget_ratio_max, 1.5); // maximum allowed setting
    }
    ...
    if ( high_temptarget_raises_sensitivity && profile.temptargetSet && target_bg > normalTarget
        ...
        if (c+target_bg-normalTarget > 0)  {
          ...
          // limit sensitivityRatio to profile.temptarget_ratio_max (1.2x by default).
          // This will enable sensitivityRatio to exceed autosens_max, so that low_temptarget_lowers_sensitivity can be used while autosens is disabled (autosens_max = 1.0).
          sensitivityRatio = Math.min(sensitivityRatio, profile.temptarget_ratio_max, 1.5); // (was previously limited to Math.min(sensitivityRatio, profile.autosens_max) )
          sensitivityRatio = round(sensitivityRatio,2);
        } else {sensitivityRatio = Math.min(profile.temptarget_ratio_max, 1.5)} // limit to max 1.5 also if (c+target_bg-normalTarget <= 0) 
    ...
    if (iob_data.iob > max_iob) {
        ...
        if (microBolusAllowed && enableSMB && bg > threshold) {
            ...
            if ( !profile.use_autoisf ) {
                ...
                smbreason = "SMB Delivery Ratio: " + round(smb_ratio,2) + ", ";
                //rT.reason += smbreason;
              }
            if ( smb_ratio > 0.5) {
