...
var determine_basal = function determine_basal(glucose_status, currenttemp, iob_data, profile, autosens_data, meal_data, tempBasalFunctions, microBolusAllowed, reservoir_data, currentTime, pumphistory, preferences, basalprofile, tdd, tdd_averages) { 
    ...
    if (pumpData > 21) {
        ...
        if (weightedAverage > 0 && average_total_data > 0) {
            tddReason = ", TDD: " + round(tdd,1) + "U " + (weightPercentage * 100) + "% W.avg: " + round(weightedAverage,1) + "U" + " 14day: " + round(average_total_data,1) + "U";
    ...
    const cr_before = round(profile.carb_ratio, 2);
    var log_isfCR = "";
    var limitLog = "";
    var log_basalAdjust = "";
    ...
    if (profile.tddAdjBasal == true) {
        log_basalAdjust = " BasalAdj: On";
        } else {
            log_basalAdjust = " BasalAdj: Off";
        }

        // Respect minCRratio and maxCRratio limits for TDD adjusted CR
        var crRatio = (tdd24h_14d_Ratio-1) * crScaleFactor + 1 // crRatio adjustable by crScaleFctor to reduce or increase impact of TDD
        if (crRatio > maxCRratio) {
            logCRratioLimited = ". TDD adjusted CR hit limit by maxCRratio setting: " + maxCRratio + " (" +  crRatio.toPrecision(4) + ")" + ", CR: " + (profile.carb_ratio / maxCRratio).toPrecision(3) + " g/U ";
            crRatio = maxCRratio;
        } else if (crRatio < minCRratio) {
            logCRratioLimited = ". TDD adjusted CR hit limit by minCRratio setting: " + minCRratio + " (" +  crRatio.toPrecision(4) + ")" + ", CR: " + (profile.carb_ratio / minCRratio).toPrecision(3) + " g/U ";
            crRatio = minCRratio;
        }
        ...
        if (useDynamicCR == true) {
            log_isfCR += "On";
            cr = round(cr/crRatio,1); // TDD adjusted CR 
            profile.carb_ratio = cr;
            logCRratio = " TDD-corrected CR is on, crRatio = " + crRatio.toPrecision(4) + ", CR scale factor: " + crScaleFactor + ", TDD-corrected CR: " + cr.toPrecision(3) + " g/U";
        } else {
            log_isfCR += "Off";
        ...
        if (!enable_sigmoid) {
            log += ", Dynamic autosens.ratio set to " + round(newRatio,2) + " with ISF: " + isf.toPrecision(3) + " mg/dl/U (" + (isf * 0.0555).toPrecision(3) + " mmol/l/U)" + log_basalAdjust + log_isfCR;
        } else { log += sigmoidLog }

        // Set the new ratio
        autosens_data.ratio = newRatio;

        logOutPut += startLog + bgLog + afLog + formula + log + logCRratio + logCRratioLimited + weightLog;
    ...
    if (!dynISFenabled && !useDynamicCR) {
        ...
        tddReason += basal_ratio_log + log_basalAdjust + log_isfCR + formula + afLog + limitLog; 
    }
    else if (dynISFenabled && !profile.tddAdjBasal) { tddReason += basal_ratio_log + log_basalAdjust + log_isfCR + formula + afLog + limitLog; + formula + limitLog; }
...
//    var sensitivityRatio;
    var temptargetRatio = 1.0 // variable for TT-adjusted dISF
    ...
    if ( high_temptarget_raises_sensitivity && profile.temptargetSet && target_bg > normalTarget ||
        ...
        if (c+target_bg-normalTarget > 0)  {
          ...
          temptargetRatio = sensitivityRatio; // use the original calculation of sensitivityRatio for TT-adjusted dISF
    ...
    if (profile.temptargetSet && target_bg < normalTarget && dynISFenabled) {
            autosens_data.ratio = temptargetRatio * newRatio; // use the original calculation of sensitivityRatio for TT-adjusted dISF, scale ISF regardless of newRatio
    ...
    if (sensitivityRatio && !dynISFenabled) { // Disable adjustment of basal by sensitivityRatio when using dISF
        ...
        basal = profile.current_basal * tdd24h_14d_Ratio * temptargetRatio; // Adjust basal by both tdd24h_14d_Ratio and temptargetRatio when dISF and tddAdjBasal is used
        basal = round_basal(basal, profile);
        process.stderr.write("TDD- and temptarget adjustment of basals activated, using tdd24h_14d_Ratio " + round(tdd24h_14d_Ratio,2) + ", TDD 24h = " + round(tdd_before,2) + "U, Weighted average TDD = " + round(weightedAverage,2) + "U, (Weight percentage = " + weightPercentage + "), Total data of TDDs (up to 14 days) average = " + round(average_total_data,2) + "U. " + "temptargetRatio = " + round(temptargetRatio,2) );
        ...
        basal = profile.current_basal * temptargetRatio;
        basal = round_basal(basal, profile); // Adjust basal by temptargetRatio when dISF is used
        process.stderr.write("Temptarget adjustment of basals activated, using temptargetRatio = " + round(temptargetRatio,2) );
        if (basal !== profile_current_basal) {
            process.stderr.write("Adjusting basal from " + profile_current_basal + " U/h to " + basal + " U/h; ");
        } else { process.stderr.write("Basal unchanged: " + basal + " U/h; "); }
    }
...
// Initialize and calculate variables used for predicting BGs
    ...
    try {
        iobArray.forEach(function(iobTick) {
            ...
            var predUAMBGI = round(( -iobTick.activity * profile.sens / temptargetRatio * 5 ), 2); // Removing dynISF from UAM predictions. Use ISF = profile.sens / temptargetRatio for UAM predBGI to avoid extreme predictions caused by dISF, while allowing sensitivity adjustment by high or low temp targets.
            ...
            switch(true) {
                case dynISFenabled && !enable_sigmoid:
                    ...
                    UAMpredBG = UAMpredBGs[UAMpredBGs.length-1] + predUAMBGI + Math.min(0, predDev) + predUCI; // use ISF = profile.sens / temptargetRatio for UAM predBGI to avoid extreme predictions caused by dISF, while allowing sensitivity adjustment by high or low temp targets.
                    console.log("Dynamic ISF (Log formula) adjusted prediction for UAM: UAMpredBG: " + round(UAMpredBG,2));
                    break;
                default:
                    UAMpredBG = UAMpredBGs[UAMpredBGs.length-1] + predBGI + Math.min(0, predDev) + predUCI;
            }  
