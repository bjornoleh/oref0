...
function defaults ( ) {
  return /* profile */ {
    max_iob: 4 // if max_iob is not provided, will default to zero
    ...
    , maxSMBBasalMinutes: 60 // maximum minutes of basal that can be delivered as a single SMB with uncovered COB
    , maxUAMSMBBasalMinutes: 60 // maximum minutes of basal that can be delivered as a single SMB when IOB exceeds COB
    , SMBInterval: 2 // minimum interval between SMBs, in minutes.
    , bolus_increment: 0.05 // minimum bolus that can be delivered as an SMB
    , maxDelta_bg_threshold: 0.27 // maximum change in bg to use SMB, above that will disable SMB
    , curve: "ultra-rapid" // default = rapid-acting, change this to "ultra-rapid" for Fiasp, or "bilinear" for old curve
    , useCustomPeakTime: true // allows changing insulinPeakTime
    , insulinPeakTime: 55 // number of minutes after a bolus activity peaks.  defaults to 55m for Fiasp if useCustomPeakTime: false
    ...
    , adjustmentFactor: 1 //Default value: 1
    , useNewFormula: false // Default value: false.
    , enableDynamicCR: false // Default value: false.
    , enableChris: false // Default value: false.
    ...
    , crSensRefBG: 100 // Default value: 100. Reference BG level (mg/dL) for TDD based sensitivity corrected carb ratio
    , crScaleFactor: 0 // Default value: 0. Scales crRatio for TDD-adjusted CR. 1 = unscaled. Lower the number to reduce the impact of TDD variation,  0 = disabled. Suggested settings: 0,2 - 0,5
    , minCRratio: 1 // Default = 1 disables strengthening (lowering) of CR. Increase the number to allow a stronger CR by automatic TDD adjustment (require middleware change in FAX).
    , maxCRratio: 1 // Default = 1 disables weakening (raising) of CR. Lower the number to allow a weaker CR by automatic TDD adjustment (require middleware change in FAX).
    , averageTDD: 0 // enter average TDD manually for now
    , weightingfactor24hTDD: 1 // weighting factor for the 24 h TDD (0 - 1)
  };
}
...
function displayedDefaults () {
    ...
    profile.crSensRefBG = allDefaults.crSensRefBG;
    profile.crScaleFactor = allDefaults.crScaleFactor;
    profile.minCRratio = allDefaults.minCRratio;
    profile.maxCRratio = allDefaults.maxCRratio;
    profile.averageTDD = allDefaults.averageTDD;
    profile.weightingfactor24hTDD = allDefaults.weightingfactor24hTDD;
